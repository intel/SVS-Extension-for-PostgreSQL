...
VALGRIND_CMD=(
    ...
    --error-exitcode=1
    --num-callers=30
    --max-threads=500    # SVS creates hardware_concurrency() threads per build; 2000 allows ~8 builds on a 224-vCPU host
    --log-file="$VALGRIND_LOG_PATTERN"
    "${SUPP_ARGS[@]}"
    "$POSTGRES_BIN"
    -D "$PG_DATA_DIR"
    -p 5433                   # use port 5433 to avoid conflicts
...
SOCKET="$VALGRIND_DIR/.s.PGSQL.5433"
...
for i in $(seq 1 120); do
    ...
    if [ -S "$SOCKET" ]; then
        ...
        if "$PSQL" -h "$VALGRIND_DIR" -p 5433 -d postgres -c "SELECT 1" >/dev/null 2>&1; then
            echo " ready (${i}s)"
            READY=true
            break
        fi
...
PSQL_CONN=(-h "$VALGRIND_DIR" -p 5433 -d postgres)
...
# Now using enhanced test suite with coverage for new features
# ---------------------------------------------------------------------------
VAMANA_TEST_SQL="$VALGRIND_DIR/vamana_valgrind_test.sql"
ENHANCED_SQL="$SCRIPT_DIR/vamana_valgrind_test_enhanced.sql"

# Copy the enhanced SQL test suite
if [ -f "$ENHANCED_SQL" ]; then
    cp "$ENHANCED_SQL" "$VAMANA_TEST_SQL"
    echo "✓ Using enhanced SQL test suite with new features coverage"
else
    echo "WARNING: Enhanced SQL test file not found, generating basic tests..."
