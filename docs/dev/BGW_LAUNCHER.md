...
An array-wide lock guards slot identity (finding a slot by OID, reserving a new slot) and the launcher's backoff fields. Because it guards identity, a write into an entry belongs inside the lock, and every mutator reached from a backend makes it there. One launcher path does not yet: the CPU-grant pass resolves each enabled database's entry, computes a budget, then writes the grants back afterwards, outside the lock. That is safe today only because the launcher is itself the sole process that releases a reserved entry, so nothing can recycle one while it is mid-pass — an argument about who the callers are rather than a property the lock enforces, and it is deliberately left to its own change. The fields are atomics, so each write is untorn on its own, but only the lock keeps a write aimed at the database it was meant for: an entry that is found and then written to after the lock is released may by then have been freed and re-reserved for another database, and the write lands on that database's control block instead. Writes take the lock in *shared* mode — identity changes only under exclusive, and two writers of the same field are already ordered by its atomic — so incrementing the index counter still does not serialize `CREATE INDEX` in one database against another. It serializes only against reservation and release, which are rare and administrative.
