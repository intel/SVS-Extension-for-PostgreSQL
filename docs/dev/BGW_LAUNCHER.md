...
2. **Drain pending notifications, outside any transaction.** The notification payload is inspected here. An empty payload means "the table changed, re-diff." A restart command payload names a specific database to bounce (see [Section 9](#9-drain-and-stop-pause-restart-and-clean-shutdown)). Draining must happen outside a transaction, and must be done in a mode that does not attempt to flush to a client connection, because the launcher has no client; flushing would error and crash the launcher on every catalog change. Draining is also necessary in its own right, to advance the notification queue and avoid a slow resource leak on a busy table.
...
- **Explicit restart**, triggered by a dedicated function. This lets an operator bounce one database's worker on demand, for example to force a clean checkpoint and slot advance right now rather than waiting for the debounced checkpoint policy. The restart function sends an explicit *verb* on the same notification channel the launcher already listens on: a payload that names the database and asks for a restart, rather than a state the launcher has to infer from a diff. On receiving it, the launcher runs a defined shutdown-then-respawn sequence for that one worker: signal it to drain and stop, wait for it to exit, re-read its row, and respawn it as a normal start. Because the drain always checkpoints first, the restarted worker comes back to current on-disk state and an idle slot, with nothing to replay.
...
Only enablement and existence changes go through the reconcile diff. The restart verb is handled as a one-off command on receipt and is not folded into the diff logic.
