...
**Confidentiality.** Stored embeddings (on disk in `$PGDATA/vamana_indexes/` and in BGW memory) follow PostgreSQL's `$PGDATA` permissions posture, which `SetDataDirectoryCreatePerm` derives from the mode of `$PGDATA` itself; the umask the postmaster inherits is discarded (`postmaster.c` `umask(PG_MODE_MASK_OWNER)`, then `umask(pg_mode_mask)` in `miscinit.c`). Every mode the extension controls is stated at the point of creation rather than obtained by umask reduction: the three directories it creates itself, `vamana_indexes/`, `vamana_indexes/<dboid>/` and `vamana_indexes/<dboid>/<relid>/`, via `MakePGDirectory` (`pg_dir_create_mode`, `0700` or `0750` with group access), and `tidmap.bin` via `OpenTransientFilePerm` plus an `fchmod` on the still-empty file (`pg_file_create_mode`, `0600` or `0640`) — see `VamanaEnsureSaveDir` and `VamanaSaveTidMapAtomically` in `vamanaio.c`. The `pg_*_create_mode` globals are used in preference to hardcoded modes so a cluster initialized with `initdb -g` retains group access. The remaining gap is the SVS payload, which the library creates in its entirety: `svs_index_save(index, directory, error)` accepts no mode, and the library creates the `config/`, `data/` and `graph/` subdirectories requesting `0777` and their files requesting `0666`. Nothing the extension does narrows those requests — `pg_mode_mask` alone is what brings them down to `0700`/`0600`, or `0750`/`0640` with group access. Like the assumption about SVS error strings below, this is a property of the library's I/O behavior and should be rechecked on each SVS version bump. The extension does not implement access control beyond the filesystem in this release. SVS error strings contain only metadata (operation names, error codes, dimensions), not vector values; this assumption is revalidated on each SVS library version bump.

**Temporary file lifetime.** `tidmap.bin` is published atomically by writing `tidmap.bin.tmp` and renaming it. The temp file is unlinked on every failure path in `VamanaSaveTidMapAtomically`, and any pre-existing temp file is unlinked before the new one is created with `O_EXCL`. That delete-before-create is a correctness requirement as well as cleanup: `O_CREAT` does not alter the mode of a file that already exists, so a stale temp file at a wider mode would keep it and the rename would publish that mode onto `tidmap.bin`. A temp file orphaned by a crash between the write and the rename is swept at the top of `VamanaLoadTidMap`, gated on `st_mtime < PgStartTime` so a concurrent writer's temp file is never removed between its close and its rename.
