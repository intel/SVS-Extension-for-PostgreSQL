...
{
    my $node = PostgreSQL::Test::Cluster->new('vamana_reload_scope');
    $node->init;
    $node->append_conf('postgresql.conf', "shared_preload_libraries = 'svs'");
    $node->append_conf('postgresql.conf', "wal_level = logical");
    ...
    {
        $node->safe_psql('postgres', qq{
            CREATE TABLE rs_tbl_$i (id serial PRIMARY KEY, val vector($dim));
            INSERT INTO rs_tbl_$i (val)
                SELECT ARRAY[$array_sql]::vector
                FROM generate_series(1, 20);
            CREATE INDEX rs_tbl_${i}_idx ON rs_tbl_$i USING vamana (val vector_l2_ops);
        });
    }
    ...
    for my $i (1 .. 3)
    {
        $node->safe_psql('postgres', qq{
            SET enable_seqscan = off;
            SELECT id FROM rs_tbl_$i ORDER BY val <-> '[$query_sql]' LIMIT 1;
        });
        my $r = $node->safe_psql('postgres', "SELECT 'rs_tbl_${i}_idx'::regclass::oid;");
        chomp $r;
        $relid{$i} = $r;
    }

    my $offset = -s $node->logfile;
    $node->safe_psql('postgres', "TRUNCATE rs_tbl_2;");
    usleep(500_000) for (1 .. 10);

    my $delta = substr(slurp_file($node->logfile), $offset);
    my ($r1, $r2, $r3) = ($relid{1}, $relid{2}, $relid{3});

    ok($delta =~ /evicting vamana cache entry for relation $r2\b/,
        'truncated index was evicted');
    ok($delta !~ /evicting vamana cache entry for relation $r1\b/,
        'untouched sibling index was NOT evicted');
    ok($delta !~ /evicting vamana cache entry for relation $r3\b/,
        'most-recently-cached sibling index was NOT evicted');
    ...
    unlike($delta2, qr/loading vamana index $r1\b/, 'sibling t1 stayed cached across the truncate');
    unlike($delta2, qr/loading vamana index $r3\b/, 'sibling t3 stayed cached across the truncate');
